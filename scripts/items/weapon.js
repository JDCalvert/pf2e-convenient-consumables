...
/**
 * Find the instances of the alchemical weapons in the actor's inventory, allow the player to
 * choose one, and then post it to chat.
 * 
...
export async function postConsumableWeapon(title, header, ...choices) {
    ...
    const matchingWeapons = actor.itemTypes.weapon.filter(weapon => allSlugs.includes(weapon.slug) && weapon.quantity);
    if (!matchingWeapons.length) {
    ...
    for (const choice of choices) {
        const consumables = matchingWeapons.filter(consumable => consumable.slug === choice.slug);
        choicesBySlug.set(
            choice.slug,
            {
                choice: choice,
    ...
    const strike = actor.system.actions.find(action => action.item.id === item.id)
    const action = strike?.auxiliaryActions?.find(action => action.carryType === "held")
    
    await action?.execute();
    
    // Now we've drawn the weapon, we need to find the drawn version
    const weapon = actor.itemTypes.weapon.find(weapon => weapon.isEquipped && weapon.slug == item.slug && isInfused(weapon) === isInfused(item));
    if (!weapon) {
        return;
    }
